 ...
 */
static int si7021_get_rh_and_temp(struct si7021_data)
{
    int retval;
    ...
    retval = i2c_write_read(si7021_data->i2c_dev, DT_INST_REG_ADDR(0), &command, 1, buffer, sizeof(buffer));
    ...
    #if CONFIG_SI7021_ENABLE_CHECKSUM
    if (crc8(buffer, SI7021_DATA_SIZE, SI7021_CRC_POLY, SI7021_CRC_INIT, false) != temp[2])
    {
        LOG_ERROR("Error with checksum.");
        return -EIO;
    }
    #endif 

    si7021_data->rh_code = buffer[0] << 8 | buffer[1];

    /* We can reuse the same buffer, and now get the temperature taken during the humidity measurement */
    command = SI7021_READ_TEMP_FROM_PREV_RH;
    ...
    si7021_data->temp_code = buffer[0] << 8 | buffer[1];
}
 ...
 */
static int si7021_sample_fetch(const struct device *dev, enum sensor_channel chan, struct sensor_value *val)
{
    int retval;
...
static int si7021_init(const struct device *dev)
{
    ...
    if (!drv_data->i2c_dev)
    {
        LOG_ERROR("Could not initiliase i2c.");
        return -EINVAL;
    }
